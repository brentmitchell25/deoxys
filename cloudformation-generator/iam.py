from troposphere.iam import Role, User, ManagedPolicy, Policy as Pol, InstanceProfile
...
def principalArn(principal, G, roleId):
    if principal["Owner"] == "User":
        ...
        for role in principal["Name"]:
            roles.append(
                Join("", ["arn:aws:iam::", Ref("AWS::AccountId"), ":role/", role]))
...
def resourceArn(resource):
    region = Ref(
        "AWS::Region") if "Region" not in resource else resource["Region"]
    accountId = Ref(
        "AWS::AccountId") if "AccountId" not in resource else resource["AccountId"]
...
def policy(statements, G, roleId):
    return Policy(
        Statement=[getStatement(statement, G=G, roleId=roleId)
                   for statement in statements["Statements"]]
    )


def instanceProfile(role, path=None, instanceProfileName=None):
    parameters = {
        "Path": path,
        "Roles": Ref(role),
        "InstanceProfileName": instanceProfileName
    }
    return InstanceProfile(**dict((k, v) for k, v in parameters.iteritems() if v is not None))
        ...
        for role in item['Roles']:
            ...
            if 'InstanceProfile' in role:
                ...
                if str(role['InstanceProfile']) == 'true':
                    instanceProfileResource = instanceProfile(roleResource)
                else:
                    instanceProfileResource = instanceProfile(
                        roleResource, role['InstanceProfile']['Path'], role['InstanceProfile']['InstanceProfileName'])
                utilities.mergeNode(G, id=instanceProfileId, resource=instanceProfileResource, image=roleImg,
                                    name=role["RoleName"] + 'InstanceProfile')
                G.add_edge(instanceProfileId, roleId)
